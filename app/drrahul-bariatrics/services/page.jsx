    ...
    <div style={{background:P.bg}}>
      ...
      <section style={{background:"#f5f1eb",padding:"4rem 1.5rem"}}>
        <div style={{maxWidth:1000,margin:"0 auto"}}>
          ...
          <div className="pricing-grid">
            {plans.map(plan=>(
              <div key={plan.name} style={{background:plan.best?"#1A1A2E":"#fff",color:plan.best?"#fff":"#1a1a1a",borderRadius:16,padding:"2rem",textAlign:"center",boxShadow:plan.best?"0 8px 32px #1A1A2E55":"0 2px 16px rgba(0,0,0,0.06)",border:plan.best?"2px solid #E94560":"1px solid #e8e0d5",position:"relative"}}>
                {plan.best && (
                  <div style={{position:"absolute",top:-14,left:"50%",transform:"translateX(-50%)",background:"#E94560",color:"#000",padding:"0.25rem 1.2rem",borderRadius:25,fontSize:12,fontWeight:700}}>MOST POPULAR</div>
                )}
                ...
                <div style={{fontSize:36,fontWeight:700,color:plan.best?"#E94560":P.primary,marginBottom:"1.5rem"}}>{plan.price}</div>
                <ul style={{listStyle:"none",padding:0,margin:"0 0 1.5rem",textAlign:"left"}}>
                  {plan.features.map(f=>(
                    <li key={f} style={{padding:"0.35rem 0",fontSize:14,borderBottom:"1px solid rgba(255,255,255,0.15)",display:"flex",alignItems:"center",gap:8}}>
                      <span style={{color:plan.best?"#E94560":"#25D366"}}>✓</span> {f}
                    </li>
                  ))}
                </ul>
                <a href="tel:919909962801" style={{display:"block",padding:"0.8rem",borderRadius:10,textDecoration:"none",fontWeight:700,fontSize:15,background:plan.best?"#E94560":"#1A1A2E",color:plan.best?"#000":"#fff"}}>Get Started</a>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{background:`linear-gradient(135deg, ${P.dark}, ${P.primary})`,padding:"3.5rem 1.5rem",textAlign:"center"}}>
        ...
        <div style={{display:"flex",gap:"1rem",justifyContent:"center",flexWrap:"wrap"}}>
          <a href="tel:919909962801" style={{background:"#E94560",color:"#000",padding:"0.85rem 2rem",borderRadius:30,fontWeight:700,fontSize:15,textDecoration:"none"}}>📞 Free Consultation</a>
          <a href="https://wa.me/919909962801?text=Hi%2C%20I%20found%20you%20on%20Google%20and%20I%27m%20interested%20in%20your%20services." target="_blank" style={{background:"#25D366",color:"#fff",padding:"0.85rem 2rem",borderRadius:30,fontWeight:700,fontSize:15,textDecoration:"none"}}>💬 WhatsApp Us</a>
        </div>
      </section>
    </div>
  )
